...
# FZF configuration
$FZF_DEFAULT_HEADER_WITH_MULTI = '(Multi-select) Select items with TAB and ENTER to confirm'
$FZF_DEFAULT_HEADER_WITH_SINGLE = '(Single-select) Select item with ENTER to confirm'
$FZF_PREFIX_PROMPT = '🔍 '
...
$env:FZF_DEFAULT_OPTS = "--color=`"$FZF_COLOR_MOLOKAI`" --ansi --cycle --border=rounded --prompt=`"$FZF_PREFIX_PROMPT`" --pointer=$POINTER --marker=$MARKER --header=`"$FZF_DEFAULT_HEADER_WITH_SINGLE`" --multi=0 --bind=`"$FZF_DEFAULT_BIND`""
